...
import {BaseButtonProps, useModal} from "qore-components";
...
const CreateOrUpdateTransactionPartyModal = ({
    ...
    const transactionPartyModalConfig = {
        ...
        submitBtnProps: {text: isUpdate ? "Update" : "Create Transaction Party"} as BaseButtonProps,
        onSuccessfulSubmission: () => {
            modal?.close()
        },
        initialValues: isUpdate ? {...updateTransactionPartyRequestInit, ...data} : {...createTransactionPartyRequestInit, ...data}
    };
    const modalTitle = isUpdate ? "Update Transaction Party" : "Create Transaction Party";

    return (
        <BaseModalLayout modalTitle={modalTitle} childrenContainerStyle={{padding: "1em", maxHeight: "500px"}}>
            <CreateOrUpdateTransactionPartyForm {...transactionPartyModalConfig}
                                                onSuccessfulSubmission={transactionPartyModalConfig.onSuccessfulSubmission}/>
        </BaseModalLayout>
