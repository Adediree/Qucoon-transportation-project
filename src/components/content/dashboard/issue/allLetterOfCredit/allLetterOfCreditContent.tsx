...
import {BaseDataGrid, BaseDataGridProps, useModal} from "qore-components";
import {
...
const AllLetterOfCreditContent = () => {
    ...
    const [deleteLetterOfCreditByLetterOfCreditId,] = useDeleteLetterOfCreditByLetterOfCreditIdMutation();
    const createOrUpdateLetterOfCreditModal = useModal(ModalEnum.CreateOrUpdateLetterOfCreditModal);
        ...
        await confirmationModal.open({
            ...
            endBtnProps: {
                text: "Delete", onClick: async () => {
                    await deleteLetterOfCreditByLetterOfCreditId({letterOfCreditId: data?.letterOfCreditId}).unwrap();
