...
const handleApiError = (
    ...
    if (message.includes('JWT') || message.includes('Authorization is required')) {
        toastUtil.showUniqueToast('session-expired', "Session expired, kindly login", "error");
    ...
    switch (status) {
        ...
        case 'TIMEOUT_ERROR':
            ...
            break;
        default:
            if ((message && message !== defaultMessage) && !(expectedResponseCodes && responseCode && expectedResponseCodes.includes(responseCode))) {
                ModalUtil.getInstance().open(ModalEnum.AppErrorModal, {message: message, title: "Error Occurred"});
